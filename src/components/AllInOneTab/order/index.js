...
export const MobOrder=()=>{
    return (
        <>
        <div className="flex bg-red-300 py-4 items-center justify-evenly md:hidden">
            <GiScooter className="w-12 h-12 text-blue-700"/>
            <div>
                <p >Order Online</p>
                <p className="text-xs text-white bg-blue-600 w-max p-0.5">30% Off</p>
            </div>
            <div onClick={LaunchRazorPay}><BsFillArrowRightCircleFill className="w-6 h-6"/></div>
        </div>
        </>
    )
}
...
const LapOrder = () =>{
    return (
        ...
        <button className="text-white bg-red-500 text-semibold p-2 hover:bg-red-600 rounded-md my-2" onClick={LaunchRazorPay}>Order Now!</button>
        </>
    )
}
const Order = () => {
